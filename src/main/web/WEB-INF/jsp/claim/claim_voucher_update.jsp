...
<script type="text/javascript">
			...
			$("#AddRow").click(function(){		
				...
				//传递一共添加多少问题 
				rowNumber++;
				$("#rowNumber").attr("value", rowNumber);
				$("#account").attr("value","");
				$("#desc").attr("value","");
		...
		function delRow(id){
			var DetailsId = $("#id"+id).val();
			if(typeof(DetailsId) != 'undefined'){
				var num = $("#num").val();
				$("#DetailsId").append("<input type='hidden' name='voucherDetailsIdList["+num+"]' value='"+DetailsId+"'/>");
			...
			var rowNumber=$("#rowNumber").val();
				...
				$("#id"+s).attr("name","detailList["+(s-1)+"].id");
				$("#id"+s).attr("id","id"+(s-1));
...
<body>
	<div class="action  divaction">
		...
		<div class="pages">
			<s:form action="claimVoucher!updateClaimVoucher.action"
				name="claimForm">
				<div id="DetailsId">
				...
				<input type="hidden" id="claimId" name="claimVoucher.id"
					value="<s:property value="claimVoucher.id"/>" />
				<table width="90%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						...
						<td>部&nbsp;&nbsp;门：<s:property
								value="claimVoucher.sysEmployeeByCreateSn.sysDepartment.name" />
						</td>
						<td>职&nbsp;&nbsp;&nbsp;&nbsp;位：<s:property
								value="claimVoucher.sysEmployeeByCreateSn.sysPosition.nameCn" />
						</td>
					</tr>
					<tr>
						<td>总金额： <s:textfield name="claimVoucher.totalAccount"
								id="totalAccount" readonly="false" /></td>
						<td>填报时间：<input type="hidden" name="claimVoucher.createTime"
							value="<s:property value="claimVoucher.createTime"/>" /> <s:date
								name="claimVoucher.createTime" format="yyyy-MM-dd HH:mm:ss" />
						</td>
						<td>状态：<s:property value="claimVoucher.status" /></td>
						<td>待处理人：<s:property
								value="claimVoucher.sysEmployeeByNextDealSn.name" /></td>
						<s:hidden id="status" name="claimVoucher.status"
							value="%{claimVoucher.status}" />
					</tr>
					<tr>
						<td colspan="4"><p>-------------------------------------------------------------------------------</p>
						</td>
				...
				<table id="myTable" width="90%" border="0" cellspacing="0"
					...
					<s:iterator value="claimVoucher.bizClaimVoucherDetails"
						id="claimDetail" begin="0" status="s">
						<tr>
							<td><s:hidden id="id%{#s.index}"
									name="detailList[%{#s.index}].id" value="%{#claimDetail.id}" />
								<s:hidden id="item%{#s.index}"
									name="detailList[%{#s.index}].item"
									value="%{#claimDetail.item}" /> <s:property
									value="#claimDetail.item" /></td>
							<td><s:hidden id="account%{#s.index}"
									name="detailList[%{#s.index}].account"
									value="%{#claimDetail.account}" /> <s:property
									value="#claimDetail.account" /></td>
							<td><s:hidden id="desc%{#s.index}"
									name="detailList[%{#s.index}].des" value="%{#claimDetail.des}" />
								<s:property value="#claimDetail.des" />
							</td>
							<td><img src=${images}/delete.gif width=16 height=16
								id=DelRow${s.index} onclick="delRow(${s.index})" />
							</td>
						</tr>
					</s:iterator>
				</table>
				<table id="detailTable" width="90%" border="0" cellspacing="0"
					...
					<tr>
						...
						<td width="30%"><input type="text"
							name="claimVoucherDetail.des" id="desc" /><span class=notice>*</span>
						</td>
				...
				<table>
					<tr>
						<td>*事由：</td>
						<td colspan="3"><s:textarea name="claimVoucher.event"
								id="event" rows="5" cols="66" />
						</td>
					</tr>
